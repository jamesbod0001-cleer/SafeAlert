...
describe('Zones', () => {
  ...
  test('POST /zones → 201 creates new zone', async () => {
    ...
    expect(res.body.zone.severity).toBe('medium');
    expect(res.body.zone.verified).toBe(false);
  });

  test('POST /zones → 400 with coordinates outside Nigeria', async () => {
    const res = await request(app)
      .post(`${BASE}/zones`)
      .send({ lat: 51.5, lng: -0.1, type: 'kidnapping', device_id: 'test-device-002' });
    expect(res.status).toBe(400);
  });
...
describe('USSD', () => {
  ...
  test('POST /ussd → END session on report submission', async () => {
    ...
    expect(res.text).toContain('Report logged');
  });

  test('POST /ussd → route check first menu route', async () => {
    const res = await request(app)
      .post(`${BASE}/ussd`)
      .send({ sessionId: 'sess-004', serviceCode: '*384*911#', phoneNumber: '+2348012345678', text: '2*1' });
    expect(res.status).toBe(200);
    expect(res.text).toContain('Abuja');
    expect(res.text).toContain('Kaduna');
  });
